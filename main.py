...
@app.route('/')
def home():
    return render_template("index.html", mode=DARK[0], settings=settings)


@app.route('/<color>')
def colored(color):
    return render_template("index.html", mode=color, settings=settings)
...
def hra2(color, game, speed):
    ...
    return render_template("game2.html",
                           game=is_game_on(game), speed=speed, mode=colormode[0], kruhy=colormode[2], settings=settings)
...
def hra3(color, game):
    ...
    return render_template("game3.html",
                           game=is_game_on(game), mode=colormode[0], kruhy=colormode[3], settings=settings)


@app.route('/<color>/info')
def info(color):
    return render_template("info.html", mode=get_colormode(color)[0], settings=settings)
